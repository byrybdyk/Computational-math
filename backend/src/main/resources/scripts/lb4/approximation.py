from valida_input import validate_pairs, try_to_convert_to_int
from output import (
    print_linear,
    print_sqr,
    print_qubic,
...
def approximation():
    ...
    try:
        ...
        pass
    find_best_method(answers)

    print_linear(linear_answer)
    print_sqr(sqr_answer)
    try:
        print_qubic(cubic_answer)
    except TypeError:
        print()
    try:
        print_log(log_answer)
    except TypeError:
        print()
    try:
        print_exp(exp_answer)
    except TypeError:
        print()
    try:
        print_gradual(gradual_answer)
    except TypeError:
        print()
...
def linear_approximation(pairs):
    ...
    return [S2, compare_sqr, f"P1(x) = {a}x + {b}", fi, compare, pirson, pi]
...
def sqr_approximation(pairs):
    ...
    for pair in pairs:
        ...
        compare_sqr += e * e
    S2 = (compare_sqr / len(pairs)) ** 0.5
    return [S2, compare_sqr, f"P1(x) = {a0}x^2 + {a1}x + {a2}", fi, compare, pi]
...
def cubic_approximation(pairs):
    ...
    return [S2, compare_sqr, f"P1(x) = {a0}x^3 + {a1}x^2 + {a2}x + {a3}", fi, compare]
...
def log_approximation(pairs):
    try:
        ...
        return [S2, compare_sqr, f"P1(x) = {a}ln(x) + {b}", fi, compare, pirson, pi]
    except ValueError:
        print("There are nums < 0, cannot solve logarithmic approximation zz")
...
def exp_approximation(pairs):
    try:
        ...
        print("There are 0 in solution, cannot solve exp approximation zz")
...
def gradual_approximation(pairs):
    try:
        ...
        print("There are 0 in solution, cannot solve gradual approximation zz")
